    ...
    try {
      ...
      if (link_stat.isSymbolicLink()) {
        ...
        if (force) {
          ...
          unlinkSync(link_path)
          symlinkSync(relative_path, link_path)
          return done(`Replaced file ${link_path} with a symbolic link to ${relative_path}.`)
        }
        return fail(`Failed to create link ${link_path} because a file already exists at that location. Use --force to overwrite.`)
      } else {
        return fail(`Failed to create link ${link_path} because something other than a symbolic link or a file exists at that location. Manually inspect and move or remove it.`)
      }
    } catch (error) {

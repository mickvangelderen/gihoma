...
it('should install', () => {
      ...
      'fs': {
        ...
        statSync: path => {
          switch (path) {
            ...
            case 'gihoma/directory-already-exists': // Fall through.
        ...
        lstatSync: path => {
          switch (path) {
            ...
            case '.git/hooks/directory-already-exists':
    ...
    expectDeepEqual(install(), [
      {
        ...
        success: false,
        message: 'Failed to create link .git/hooks/file-already-exists because a file already exists at that location. Use --force to overwrite.',
        ...
        success: false,
        message: 'Failed to create link .git/hooks/directory-already-exists because something other than a symbolic link or a file exists at that location. Manually inspect and move or remove it.',
        name: 'directory-already-exists',
        hook_path: 'gihoma/directory-already-exists',
        link_path: '.git/hooks/directory-already-exists',
        relative_path: '../../gihoma/directory-already-exists'
      }
    ])
  }
  ...
  // Test --force to overwrite a link.
  {
    const install = mocked_require('./install', {
      'fs': {
  ...
  // Test --force to overwrite a file.
  {
    const install = mocked_require('./install', {
      'fs': {
        readdirSync: () => [ 'pre-commit.sh' ],
